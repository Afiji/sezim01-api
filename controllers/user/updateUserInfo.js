...
export const editProfileController = async (req, res) => {
  const { name, email, password, avatar } = req.body; // Добавлено поле avatar
  const userId = req.user.userId;

  const updatedData = {};
  if (email) updatedData.email = email;
  if (name) updatedData.name = name;
  if (avatar) updatedData.avatar = avatar; // Добавляем URL аватара к обновляемым данным
  if (password) {
    updatedData.hash_pass = await bcrypt.hash(password, 10);
  }
  ...
  // Создаем новый токен с обновленной информацией пользователя
  const token = jwt.sign(
    { email: updatedUser.email, userId: updatedUser._id },
    process.env.JWT_SECRET
  );
